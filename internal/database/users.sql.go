...
const setChirpyRedTrue = `-- name: SetChirpyRedTrue :one
UPDATE users
...
func (q *Queries) SetChirpyRedTrue(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, setChirpyRedTrue, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
